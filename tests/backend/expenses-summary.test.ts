...
describe('Expenses Summary API', async () => {
  ...
  it('should fetch expense summary', async () => {
    if (!process.env.DATA_DIR || !process.env.DATA_DIR.includes('temp-data')) {
      throw new Error('Test must run with temporary DATA_DIR');
    }
    ...
    const summary: any = await $fetch(
      '/api/expenses/summary?startDate=2023-01-01&endDate=2023-12-31'
    );
