...
export default defineEventHandler(async (event) => {
  ...
  for (const year of years) {
    ...
    const descriptions = await db.all("SELECT DISTINCT description FROM expenses WHERE description IS NOT NULL AND description != ''");
    allDescriptions = allDescriptions.concat(descriptions.map((d: any) => d.description));
  }
