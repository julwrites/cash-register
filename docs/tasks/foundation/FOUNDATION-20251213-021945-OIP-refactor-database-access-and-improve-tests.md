...
## Requirements
1.  **Refactor DB Logic**:
    *   Remove unnecessary `Promise` wrappers around `better-sqlite3` calls.
    *   Standardize on a Singleton pattern (or cached connection pool) for database connections to avoid re-opening files on every request.
2.  **Remove Dead Code**: Delete `server/api/expenses/descriptions.ts`.
3.  **Increase Test Coverage**:
    *   Add unit tests for the refactored database logic.
    *   Add integration tests for critical API endpoints (Expenses, Categories).
